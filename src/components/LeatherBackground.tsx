...
export const LeatherBackground: React.FC<LeatherBackgroundProps> = ({ disableSvgFilter = false }) => {
  return (
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden bg-[#0a0a0c]">
      ...
      {/* 2. Seamless Micro-Pebble Leather Grain Pattern (Fast CSS lightweight pattern) */}
      <div 
      ...
      {/* 3. Procedural SVG Bump-Mapped Full-Grain Leather Filter (Only rendered when not in low-end mode) */}
      {!disableSvgFilter && (
        <svg 
          className="absolute inset-0 w-full h-full opacity-35 mix-blend-soft-light pointer-events-none hidden md:block" 
      ...
      {/* 4. Fine Organic Secondary Leather Pores (Lightweight fallback) */}
      {disableSvgFilter ? (
        <div className="absolute inset-0 w-full h-full bg-gradient-to-b from-transparent via-[#060709]/40 to-[#000000]/80 pointer-events-none" />
      ) : (
        <div 
          className="absolute inset-0 w-full h-full opacity-30 mix-blend-overlay pointer-events-none hidden md:block"
          style={{
